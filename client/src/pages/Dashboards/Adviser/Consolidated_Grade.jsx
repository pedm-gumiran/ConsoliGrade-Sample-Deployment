...
export default function Consolidated_Grade() {
  const { user } = useUser();
  const adviserSections = user.advisedSections || [];
  // Fetch real data from database
  const {
    grades: initialData,

    error: gradesError,
  } = useConsolidatedGrades(adviserSections);
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <div className="text-center py-8">
            <p className="text-red-600 text-lg">
              Error loading consolidated grades: {gradesError}
            </p>
          </div>
        </div>
      </div>
  ...
  const exportToExcel = async (
    ...
    dataToExport.forEach((student) => {
      ...
      Object.keys(grades).forEach((subject) => {
        excelGrades[subject] = grades[subject] !== null ? grades[subject] : '';
      });
    ...
    saveAs(blob, `Consolidated_Grades_${quarterValue}.xlsx`);
  };
  ...
  const calculateStudentAverage = (student) => {
    ...
    const validGrades = Object.values(grades).filter((grade) => grade !== null);
  ...
  const calculateSubjectAverages = () => {
    ...
    sampleSubjects.forEach((subject) => {
      const validGrades = sortedBySex
        .map((student) => getDisplayedGrades(student)[subject])
        .filter((grade) => grade !== null);
  ...
  const columns = [
    ...
    { key: 'name', label: 'Student Name', className: 'font-medium' },
    { key: 'sex', label: 'Sex' },
    ...sampleSubjects.map((subject) => ({
      key: subject,
      label: subject,
      render: (_, row) => {
        const grade = getDisplayedGrades(row)[subject];
